<template>
  <div class="border-2 rounded-xl shadow-xl px-12 py-6 m-12">
    <div class="text-3xl font-semibold">Daftar OPD</div>

    <NuxtLink to="/admin/opd/create">
      <button
    ...
    <div>
      <DataTable
        ...
        dataKey="id"
        :loading="loading"
        :globalFilterFields="['opd', 'alamat', 'telp', 'email']"
...
const columns = [
  { field: "no", header: "No" },
  { field: "opd", header: "Nama" },
  { field: "alamat", header: "Alamat" },
  { field: "telp", header: "No. Telp" },
...
const edit = (rowData) => {
  ...
  if (selectedData.value.id !== null) {
    navigateTo("/admin/opd/edit-" + selectedData.value.id);
  }
};
...
const deleteData = (rowData) => {
  ...
  if (selectedData.value.id !== null) {
    confirm.require({
      ...
      acceptLabel: "Delete",
      rejectClass:
        "bg-slate-800 text-gray-700 border-slate-700 hover:bg-white hover:text-gray-700  ",
      acceptClass:
...
const deleteTableData = async () => {
  ...
  try {
    const response = await $fetch(
      host + "/api/assessment/admin/opd/" + selectedData.value.id,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
